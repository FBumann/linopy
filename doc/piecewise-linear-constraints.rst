...
handles all the internal bookkeeping so you can focus on the breakpoints that
define your function. Two formulations are available:

- **SOS2 (convex combination)** -- general-purpose, works with any breakpoint
  layout.
- **Incremental (delta)** -- pure LP formulation (no SOS2 or binary variables),
  requires strictly monotonic breakpoints.
...
- SOS2: the convexity ``Constraint`` (:math:`\sum \lambda_i = 1`).
- Incremental: the filling-order ``Constraint`` (or the link constraint if
  there are only 2 breakpoints).
